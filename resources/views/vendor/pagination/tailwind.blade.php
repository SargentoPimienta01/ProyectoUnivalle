...
@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Navegación de paginación') }}" class="d-flex justify-content-between align-items-center">
                ...
                <ul class="pagination">
                    ...
                    @if ($paginator->onFirstPage())
                        <li class="page-item disabled">
                            <span class="page-link" aria-label="{{ __('pagination.previous') }}">
                ...
                {{-- <span class="relative z-0 inline-flex shadow-sm rounded-md"> --}}
                        ...
                        {{-- @if (is_array($element)) --}}
                            {{-- @foreach ($element as $page => $url) --}}
                                {{-- @if ($page == $paginator->currentPage()) --}}
                                    ...
                                    {{-- </span> --}}
                                {{-- @else --}}
                                    {{-- <a href="{{ $url }}" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 hover:text-gray-500 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150" aria-label="{{ __('Ir a la página :page', ['page' => $page]) }}"> --}}
                                        {{-- {{ $page }} --}}
                                    {{-- </a> --}}
                                {{-- @endif --}}
                            {{-- @endforeach --}}
                    ...
                    {{-- @if ($paginator->hasMorePages()) --}}
                        {{-- <a href="{{ $paginator->nextPageUrl() }}" rel="next" class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-r-md leading-5 hover:text-gray-400 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150" aria-label="{{ __('pagination.next') }}"> --}}
                            {{-- <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"> --}}
                                {{-- <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10l7.707 7.707a1 1 0 01-1.414 1.414l-8-8a1 1 0 010-1.414l8-8a1 1 0 011.414 0z" clip-rule="evenodd" /> --}}
                            {{-- </svg> --}}
                        {{-- </a> --}}
                    {{-- @else --}}
